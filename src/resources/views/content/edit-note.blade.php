...
@section('content')
    <div class="container">
        <h1>Evaluation de {{ $notes->first()->salarie->nom }} &nbsp; {{ $notes->first()->salarie->prenom }}</h1>
        <form id="multiStepForm" action="{{ route('fiche-evaluation-store') }}" method="POST" data-parsley-validate>
            @csrf
            <input type="hidden" name="salarie_id" value="{{ $notes->first()->salarie->id }}" id="">
            <div class="step-pane active" id="step1" data-parsley-group="block-1">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-borderless">
                                    <thead>
                                        <tr>
                                            <th class="fs-large invert-text-dark">
                                                {{ $subCategories->first()->categorie->name }}
                                            </th>
                                        </tr>
                                    </thead>
                                    @foreach ($subCategories as $subCategorie)
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td style="width: 10%;"> <input
                                                        ...
                                                        placeholder="Note" value="{{ $note->note }}"
                                                ...
                                                <td style="width: 15%;"><input value="" type="text"
                                                        name="appreciation" placeholder="Appreciation"
                                                        aria-label="First name" style="color:black !important"
                                                        id="appreciation-{{ $subCategorie->id }}"
                                                        class="form-control @error('nom') is-invalid @enderror" disabled>
                                                </td>
                                                <td style="width: 45%;">
                                        ...
                                        <input type="hidden" value="{{ $subCategorie->id }}"
                                            name="note_sub_categorie_id[]">
                                    @endforeach
                                </table>
                            </div>
                        </div>
                ...
                <div class="row mb-3 float-center text-center" style="margin: auto">
                    <label class="col-sm-4 col-form-label fs-large"> Note
                        Finale</label>
                    <div class="col-sm-4 ">
                        <input type="text" id="finale-note" class="form-control" disabled>
                    </div>
                    <input type="hidden" name="note_final" id="note_final" value="">
                </div>

                <a class="btn btn-primary float-end next-btn">Suivant</a>
            </div>
            ...
            <div class="step-pane" id="step2" data-parsley-group="block-2">
                ...
                <div>
                    ...
                    <textarea class="form-control" name="point_fort" id="exampleFormControlTextarea1" rows="3"
                        data-parsley-maxlength="255">{{ $remarque->point_fort }}</textarea>
                </div>
                <div>
                    <label for="exampleFormControlTextarea1" class="form-label">Points à améliorer

                    </label>
                    <textarea class="form-control" name="point_ameliorer" id="exampleFormControlTextarea1" rows="3"
                        data-parsley-maxlength="255" data-parsley-group="block-2">{{ $remarque->point_ameliorer }}</textarea>
                </div>
                <div>
                    <label for="exampleFormControlTextarea1" class="form-label">Projet professionnel et perspectives

                    </label>
                    <textarea class="form-control" name="projet" id="exampleFormControlTextarea1" rows="3"
                        data-parsley-maxlength="255" data-parsley-group="block-2">{{ $remarque->projet }}
                      </textarea>
                </div>
                <div>
                    ...
                    <textarea class="form-control" name="action" id="exampleFormControlTextarea1" rows="3"
                        data-parsley-maxlength="255" data-parsley-group="block-2">{{ $remarque->action }}</textarea>
                </div>
                <div>
                    ...
                    <textarea class="form-control" name="commentaire" id="exampleFormControlTextarea1" rows="3"
                        data-parsley-maxlength="255" data-parsley-group="block-2">{{ $remarque->commentaire }}</textarea>
                </div>
    ...
    <script>
        ...
        function calculateAverage(categoryId) {
            ...
            const inputs = document.querySelectorAll(`input[type="number"][data-category="${categoryId}"]`);
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const noteInputs = document.querySelectorAll('input[type="number"][id^="note-"]');
            noteInputs.forEach(input => {
                const subCategorieId = input.id.split('-')[1];
                appreciation(subCategorieId);
            });

            const categoryIds = [...new Set([...noteInputs].map(input => input.dataset.category))];
        ...
        document.querySelector('form').addEventListener('submit', function(event) {
            ...
            fetch("{{ route('fiche-evaluation-edit', $remarque->id ) }}", {
                    method: 'POST',
                    body: formData,
                    headers: {
