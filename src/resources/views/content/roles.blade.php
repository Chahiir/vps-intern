  ...
  @include('content.modals.addRole',['permissions' => $permissions])
    ...
    <table class="table">
        ...
        <tbody>
            ...
            @include('content.modals.editRole',['permissions' => $permissions],['role'=>$role])
            <tr>
                <td>{{ $role->name }}</td>
                <td>
                    @foreach($role->permissions as $permission)
                    <span class="badge rouded-pill bg-label-info">{{ $permission->name }}</span>
                    @endforeach
                </td>
                <td>
                    <button class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#edit-{{ $role->id }}">Edit</button>
